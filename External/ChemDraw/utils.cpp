...
unsigned int get_fuse_label(Atom *atm) {
  ...
  atm->getPropIfPresent<unsigned int>(internKey(FUSE_LABEL), label);
  return label;
}
...
void set_fuse_label(Atom *atm, unsigned int idx) {
  ...
  if (idx) {
    atm->setProp<unsigned int>(internKey(FUSE_LABEL), idx);
  } else {
    atm->clearProp(internKey(FUSE_LABEL));
  }
}
...
struct FragmentReplacement {
  ...
  bool replace(RWMol &mol) {
    ...
    auto bond_ordering =
        replacement_atom->getProp<std::vector<int>>(internKey(CDX_BOND_ORDERING));
    ...
    for (auto bond : replacement_bonds) {
      ...
      unsigned bond_id = 0;
      if (!bond->getPropIfPresent<unsigned int>(internKey(CDX_BOND_ID), bond_id)) {
...
bool replaceFragments(RWMol &mol) {
  ...
  for (auto &atom : mol.atoms()) {
    ...
    if (label) {
      if (atom->hasProp(internKey(CDX_BOND_ORDERING))) {
...
Atom::ChiralType getChirality(ROMol &mol, Atom *center_atom, Conformer &conf) {
  if (center_atom->hasProp(internKey(CDX_BOND_ORDERING))) {
    auto bond_ordering =
        center_atom->getProp<std::vector<int>>(internKey(CDX_BOND_ORDERING));
    ...
    for (auto cdx_id : bond_ordering) {
      ...
      for (auto bond : mol.atomBonds(center_atom)) {
        int bond_id;
        if (bond->getPropIfPresent<int>(internKey(CDX_BOND_ID), bond_id)) {
        } else {
          return Atom::ChiralType::CHI_UNSPECIFIED;
        }
        if (bond_id == cdx_id) {
    ...
    if (center_atom->hasProp(internKey(CDX_IMPLICIT_HYDROGEN_STEREO)) &&
        center_atom->getProp<char>(internKey(CDX_IMPLICIT_HYDROGEN_STEREO)) == 'w') {
      nswaps++;
    }
...
void checkChemDrawTetrahedralGeometries(RWMol &mol) {
  ...
  for (auto atom : mol.atoms()) {
    ...
    CDXAtomCIPType cip;
    if (atom->getPropIfPresent<CDXAtomCIPType>(internKey(CDX_CIP), cip)) {
