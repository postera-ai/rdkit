...
void Synthon::setSearchMol(std::unique_ptr<ROMol> mol) {
  ...
  for (auto &atom : dp_searchMol->atoms()) {
    atom->clearProp(common_properties::molNum);
    atom->clearProp(common_properties::idx);
  }
  for (auto &bond : dp_searchMol->bonds()) {
    bond->clearProp(common_properties::molNum);
    bond->clearProp(common_properties::idx);
  }
