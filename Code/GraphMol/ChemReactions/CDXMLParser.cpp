...
void add_template(DictKey prop, std::map<int, ROMOL_SPTR> &templates,
...
CDXMLDataStreamToChemicalReactions(std::istream &inStream, bool sanitize,
  ...
  for (size_t i = 0; i < mols.size(); ++i) {
    ...
    if (mols[i]->getPropIfPresent(common_properties::CDX_SCHEME_ID, scheme) &&
        mols[i]->getPropIfPresent(common_properties::CDX_STEP_ID, step)) {
  ...
  for (const auto &scheme : schemes) {
    ...
    for (auto idx : scheme.second) {
      ...
      if (mols[idx]->hasProp(common_properties::CDX_REAGENT_ID)) {
        used.insert(idx);
        make_query_atoms(*mols[idx]);
        add_template(common_properties::CDX_REAGENT_ID, reactant_templates, mols[idx]);
      } else if (mols[idx]->hasProp(common_properties::CDX_AGENT_ID)) {
        used.insert(idx);
        make_query_atoms(*mols[idx]);
        add_template(common_properties::CDX_AGENT_ID, agent_templates, mols[idx]);
      } else if (mols[idx]->hasProp(common_properties::CDX_PRODUCT_ID)) {
        used.insert(idx);
        make_query_atoms(*mols[idx]);
        add_template(common_properties::CDX_PRODUCT_ID, product_templates, mols[idx]);
      }
    }
