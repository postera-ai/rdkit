...
void iterateCIPRanks(const ROMol &mol, const DOUBLE_VECT &invars,
  ...
  std::vector<unsigned int> updated_nbr_idxs;
  updated_nbr_idxs.reserve(8);
  ...
  while (numRanks < numAtoms && numIts < maxIts &&
    ...
    for (int &index : allIndices) {
      ...
      while (beg != end) {
        ...
        updated_nbr_idxs.push_back(nbrIdx);

        // put the neighbor in 2N times where N is the bond order.
        ...
        bool is_chiral_phosphorous_special_case;
        if (bond->getBondType() != Bond::DOUBLE) {
          is_chiral_phosphorous_special_case = false;
        } else {  // double bond
          const Atom *nbr = mol[nbrIdx];
          if (nbr->getAtomicNum() != 15) {
            is_chiral_phosphorous_special_case = false;
          } else {
            unsigned int nbr_deg = nbr->getDegree();
            is_chiral_phosphorous_special_case = nbr_deg == 3 || nbr_deg == 4;
          }
        }
        ...
        if (is_chiral_phosphorous_special_case) {
          counts[nbrIdx] += 1;
        } else {
      ...
      if (updated_nbr_idxs.size() > 1) {  // compare vs 1 for performance.
        std::sort(std::begin(updated_nbr_idxs), std::end(updated_nbr_idxs),
                 [&ranks](unsigned int idx1, unsigned int idx2) {
                   return ranks[idx1] > ranks[idx2];
                  });
      }
      for (auto nbrIdx : updated_nbr_idxs) {
        unsigned int count = counts[nbrIdx];
        cipEntries[index].insert(
          cipEntries[index].end(), count, ranks[nbrIdx] + 1);
        counts[nbrIdx] = 0;
      }
      // add a zero for each coordinated H as long as we're not a query atom
      if (!mol[index]->hasQuery()) {
        cipEntries[index].insert(
          cipEntries[index].end(), mol[index]->getTotalNumHs(), 0);
      }
