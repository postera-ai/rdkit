...
void checkChiralityPostMove(const ROMol &mol, const Atom *oAt, Atom *nAt,
  ...
  if (nAt->getPropIfPresent(internKey(newBondOrder), incomingOrder)) {
  ...
  nAt->setProp(internKey(newBondOrder), newOrder, true);
  ...
  for (const auto *atom : {bgn, end}) {
    for (auto obnd : mol.atomBonds(atom)) {
      if (obnd->getIdx() != bnd->getIdx() && !obnd->getStereoAtoms().empty()) {
        obnd->setProp(internKey(molfragSaveStereo), obnd->getStereo());
    ...
    if (addDummies) {
      ...
      for (auto &stereo_atoms : nbr_bond_stereo) {
        ...
        stereo_atoms.first->setStereo(
            stereo_atoms.first->getProp<Bond::BondStereo>(internKey(molfragSaveStereo)));
        stereo_atoms.first->clearProp(internKey(molfragSaveStereo));
      }
