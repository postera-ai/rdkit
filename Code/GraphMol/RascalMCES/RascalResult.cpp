...
void RascalResult::rebuildFromFrags(
  ...
  for (const auto &f : frags) {
    for (auto atom : f->atoms()) {
      if (atom->hasProp(common_properties::ORIG_INDEX)) {
        fragAtoms.set(atom->getProp<int>(common_properties::ORIG_INDEX));
      }
    }
    for (auto bond : f->bonds()) {
      if (bond->hasProp(common_properties::ORIG_INDEX)) {
        fragBonds.set(bond->getProp<int>(common_properties::ORIG_INDEX));
      }
    }
  }
...
void RascalResult::applyMaxFragSep() {
  ...
  auto fragFragDist = [](const boost::shared_ptr<RDKit::ROMol> &frag1,
    ...
    for (auto at1 : frag1->atoms()) {
      int at1Idx = at1->getProp<int>(common_properties::ORIG_INDEX);
      for (auto at2 : frag2->atoms()) {
        int at2Idx = at2->getProp<int>(common_properties::ORIG_INDEX);
  ...
  if (deletedFrag) {
    ...
    for (auto &frag : frags1) {
      ...
      for (auto b : frag->bonds()) {
        int b_idx = b->getProp<int>(common_properties::ORIG_INDEX);
    ...
    for (auto &frag : frags1) {
      ...
      for (auto a : frag->atoms()) {
        int a_idx = a->getProp<int>(common_properties::ORIG_INDEX);
...
RDKit::ROMol *RascalResult::makeMolFrags(int molNum) const {
  ...
  for (auto &a : molFrags->atoms()) {
    if (!ainClique[a->getIdx()]) {
      ...
      a->setProp<int>(common_properties::ORIG_INDEX, a->getIdx());
    }
  }
  for (auto &b : molFrags->bonds()) {
    if (!binClique[b->getIdx()]) {
      ...
      b->setProp<int>(common_properties::ORIG_INDEX, b->getIdx());
    }
  }
