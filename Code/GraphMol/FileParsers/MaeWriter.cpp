	...
	void copyProperties(
	  ...
	  for (const auto &prop : origin.getDict().getData()) {
		...
		if (!propNames.empty() && (std::find(propNames.begin(), propNames.end(),
											 prop.first) == propNames.end())) {
		  continue;
		}

		switch (prop.second.getTag()) {
		case RDTypeTag::BoolTag: {
		  auto propName = prop.first;
		  if (!std::regex_match(prop.first, MMCT_PROP_REGEX)) {
			propName.insert(0, "b_rdkit_");
		  }

		  boolSetter(propName, idx, rdvalue_cast<bool>(prop.second));
		  break;
		}
		...
		case RDTypeTag::UnsignedIntTag: {
		  auto propName = prop.first;
		  if (prop.first == common_properties::_MolFileRLabel) {
			propName = MAE_RGROUP_LABEL;
		  } else if (!std::regex_match(prop.first, MMCT_PROP_REGEX)) {
			propName.insert(0, "i_rdkit_");
		  }

		  intSetter(propName, idx, rdvalue_cast<int>(prop.second));
		  break;
		}
		...
		case RDTypeTag::FloatTag: {
		  ...
		  realSetter(propName, idx, rdvalue_cast<double>(prop.second));
		  break;
		}
		...
		case RDTypeTag::StringTag: {
		  ...
		  stringSetter(propName, idx, rdvalue_cast<std::string>(prop.second));
		  break;
		}
		default:
		  ...
		  BOOST_LOG(rdWarningLog)
            << "WARNING: the property " << prop.first
