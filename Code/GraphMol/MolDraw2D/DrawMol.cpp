...
void DrawMol::extractSGroupData() {
  ...
  for (const auto &sg : sgs) {
    std::string typ;
    if (sg.getPropIfPresent(common_properties::sgTYPE, typ) && typ == "DAT") {
      std::string text;
      // it seems like we should be rendering FIELDNAME, but
      // Marvin Sketch, Biovia Draw, and ChemDraw don't do it
      // if (sg.getPropIfPresent(common_properties::sgFIELDNAME, text)) {
      //   text += "=";
      // };
      if (sg.hasProp(common_properties::sgDATAFIELDS)) {
        STR_VECT dfs = sg.getProp<STR_VECT>(common_properties::sgDATAFIELDS);
      ...
      if (sg.getPropIfPresent(common_properties::sgFIELDDISP, fieldDisp)) {
        ...
        if (fieldDisp[25] == 'R') {
          ...
          if (drawMol_->hasProp(common_properties::_centroidx)) {
            Point2D centroid;
            drawMol_->getProp(common_properties::_centroidx, centroid.x);
            drawMol_->getProp(common_properties::_centroidy, centroid.y);
            // opposite sign for y
  ...
  for (auto &sg : sgs) {
    ...
    if (includeAnnotations_) {
      ...
      if (sg.getPropIfPresent(common_properties::sgLABEL, label)) {
        ...
        annotations_.emplace_back(da);
      } else if (sg.getPropIfPresent(common_properties::sgTYPE, label)) {
        if (label == "GEN") {
