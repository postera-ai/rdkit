...
void adjustConjugatedFiveRings(RWMol &mol) {
  ...
  for (auto ring : mol.getRingInfo()->bondRings()) {
    ...
    for (auto bi : ring) {
      const auto bond = mol.getBondWithIdx(bi);
      bond->getBeginAtom()->setProp(common_properties::_conjugatedOrAromatic, 1, true);
      bond->getEndAtom()->setProp(common_properties::_conjugatedOrAromatic, 1, true);
...
bool isAromaticOrConjugated(const Atom &atom) {
  return atom.getIsAromatic() || atom.hasProp(common_properties::_conjugatedOrAromatic);
}
...
void adjustQueryProperties(RWMol &mol, const AdjustQueryParameters *inParams) {
  ...
  if (params.setMDLFiveRingAromaticity) {
    for (auto atom : mol.atoms()) {
      atom->clearProp(common_properties::_conjugatedOrAromatic);
    }
  }
}
}  // namespace MolOps
