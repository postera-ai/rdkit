...
inline bool streamWriteProps(
  ...
  COUNT_TYPE count = 0;
  for (const auto &elem : dict.iterValues()) {
    if (propnames.find(elem.first) != propnames.end()) {
      if (isSerializable(elem.second, handlers)) {
        count++;
      }
    }
  }
  ...
  for (const auto &elem : dict.iterValues()) {
    if (propnames.find(elem.first) != propnames.end()) {
      if (isSerializable(elem.second, handlers)) {
        ...
        //  a null op
        if (streamWriteProp(ss, elem.first, elem.second, handlers)) {
          writtenCount++;
        }
      }
    }
...
inline bool streamReadProp(std::istream &ss, std::string &key, RDValue &val,
                           bool &dictHasNonPOD,
                           const CustomPropHandlerVec &handlers = {}) {
  int version = 0;
  ...
  switch (type) {
    case DTags::IntTag:
      readRDValue<int>(ss, val);
      break;
    case DTags::UnsignedIntTag:
      readRDValue<unsigned int>(ss, val);
      break;
    case DTags::BoolTag:
      readRDValue<bool>(ss, val);
      break;
    case DTags::FloatTag:
      readRDValue<float>(ss, val);
      break;
    case DTags::DoubleTag:
      readRDValue<double>(ss, val);
      break;

    case DTags::StringTag:
      readRDValueString(ss, val);
      dictHasNonPOD = true;
      break;
    case DTags::VecStringTag:
      readRDStringVecValue(ss, val);
      dictHasNonPOD = true;
      break;
    case DTags::VecIntTag:
      readRDVecValue<int>(ss, val);
      dictHasNonPOD = true;
      break;
    case DTags::VecUIntTag:
      readRDVecValue<unsigned int>(ss, val);
      dictHasNonPOD = true;
      break;
    case DTags::VecFloatTag:
      readRDVecValue<float>(ss, val);
      dictHasNonPOD = true;
      break;
    case DTags::VecDoubleTag:
      readRDVecValue<double>(ss, val);
      dictHasNonPOD = true;
      break;
...
inline unsigned int streamReadProps(std::istream &ss, RDProps &props,
  ...
  for (unsigned index = 0; index < count; ++index) {
    std::string key;
    RDValue val;
    bool pod = false;  // moveInsert will update podstatus if needed
    CHECK_INVARIANT(streamReadProp(ss, key, val, pod, handlers),
                    "Corrupted property serialization detected");
    dict.moveInsert(key, val);
  }
